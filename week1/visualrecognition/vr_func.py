...
def parse_classes(json_data):
    ...
    """
    classes = set()
    classifiers = json_data["images"][0]["classifiers"]
    if len(classifiers) == 0:
        return classes
    for c in classifiers[0]["classes"]:
        classes.add(c["class"])
    return classes
...
def measure_accuracy(image_entries):
    ...
    """
    correct_image_entries = list(filter(lambda entry: entry["actual"] == entry["predicted"], image_entries))
    return len(correct_image_entries)/len(image_entries)
